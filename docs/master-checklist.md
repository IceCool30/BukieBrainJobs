...
| **Customer Payments & Escrow UX** | **Not Built** | Checkout modal, payment authorization, receipts, wallet, refunds. |
...
| **Customer Profile & Account Settings** | **Not Built** | Personal info, saved addresses, auth credentials, notification rules. |
...
- [ ] Personal information editing (`/profile`): Name, primary phone, email address.
- [ ] Saved addresses manager: Adding and editing service locations (e.g., Home in Lekki, Office in Victoria Island) with landmark notes.
- [ ] Security settings: Password change form, active sessions list, authentication provider linking.
- [ ] Notification preferences: Toggles for SMS, WhatsApp, Email, and In-App alerts.
- [ ] Account management: Safe account deletion and data export requests.

#### 1.2 Customer Payments & Escrow Frontend Experience
- [ ] Checkout drawer/modal triggered upon booking confirmation.
- [ ] Payment method selection UI: Card, Bank Transfer, and USSD flows matching Nigerian standards (Paystack / Flutterwave patterns).
- [ ] Payment authorization state machine: Processing, verified, failed, retry, and timeout states.
- [ ] Escrow protection tracker: Clear visual timeline showing funds held securely, technician dispatch, work inspection, and customer approval before payout.
- [ ] Digital receipts and invoices: Downloadable and printable receipts (`/receipt/[bookingId]`).
