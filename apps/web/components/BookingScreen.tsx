...
function DetailCard({ service, price, worker }: { service: string; price: string; worker: string | null }) {
  return (
    <section className="rounded-2xl bg-[#001A41] p-5 text-white shadow-[0_16px_32px_rgba(0,26,65,0.14)] sm:p-6">
      <p className="text-[11px] font-bold uppercase tracking-[0.14em] text-[#ABEEC8]">Booking details</p>
...
function ScheduleCard({ date, notes, time, onDate, onNotes, onTime }: {
  ...
  return (
    <section className="rounded-2xl border border-slate-200 bg-white p-5 shadow-[0_12px_30px_rgba(0,26,65,0.05)] sm:p-6">
      <div className="flex items-center gap-2 text-[#001A41]"><Calendar className="h-4 w-4 text-[#296A4B]" /><h2 className="font-display text-lg font-bold">When do you need it?</h2></div>
      <div className="mt-5 grid grid-cols-2 gap-2">
        {DATE_OPTIONS.map((option) => <DateOption key={option} option={option} active={date === option} onClick={onDate} />)}
      </div>
      <label className="mt-5 block text-xs font-bold text-slate-700" htmlFor="booking-arrival-time">Preferred time window</label>
      ...
      <label className="mt-5 block text-xs font-bold text-slate-700" htmlFor="booking-notes">Job details</label>
...
function AddressCard({ city, onCity }: { city: string; onCity: (value: string) => void }) {
  return (
    <section className="rounded-2xl border border-slate-200 bg-white p-5 shadow-[0_12px_30px_rgba(0,26,65,0.05)] sm:p-6">
      <div className="flex items-center gap-2 text-[#001A41]"><MapPin className="h-4 w-4 text-[#296A4B]" /><h2 className="font-display text-lg font-bold">Where is the job?</h2></div>
...
function PaymentCard({ price, method, onMethod }: { price: string; method: string; onMethod: (value: string) => void }) {
  return (
    <aside className="h-fit rounded-2xl border border-slate-200 bg-white p-5 shadow-[0_12px_30px_rgba(0,26,65,0.05)] sm:p-6 lg:sticky lg:top-6">
      ...
      <div className="mt-5 space-y-3 rounded-xl bg-[#F8F9FF] p-4 text-sm"><div className="flex justify-between text-slate-600"><span>Starting price</span><span className="font-bold text-[#001A41]">{price}</span></div><div className="flex justify-between border-t border-slate-200 pt-3 font-bold text-[#001A41]"><span>Price shown</span><span>{price}</span></div></div>
