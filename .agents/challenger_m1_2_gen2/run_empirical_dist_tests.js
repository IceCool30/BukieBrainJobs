...
const { InputField } = require('./dist/components/InputField');
const { BukiePassportBadge } = require('./dist/components/BukiePassportBadge');
const { EscrowShield } = require('./dist/components/EscrowShield');
...
function assert(condition, suite, name, failureMsg) {
  if (condition) {
...
safeRender('1.1 Default Button', () => {
  const html = stripReactComments(ReactDOMServer.renderToString(React.createElement(Button, { label: "Submit Order" })));
...
safeRender('1.2 Loading Button', () => {
  const html = stripReactComments(ReactDOMServer.renderToString(React.createElement(Button, { label: "Processing", isLoading: true })));
...
safeRender('1.3 Disabled Button', () => {
  const html = stripReactComments(ReactDOMServer.renderToString(React.createElement(Button, { label: "Disabled Action", disabled: true })));
...
safeRender('1.4 Button Variants', () => {
  const emerald = stripReactComments(ReactDOMServer.renderToString(React.createElement(Button, { label: "Pay Now", variant: "emerald" })));
  assert(emerald.includes('bg-[#296A4B]'), buttonSuite, 'Emerald variant color token', 'Emerald variant must use #296A4B');

  const accent = stripReactComments(ReactDOMServer.renderToString(React.createElement(Button, { label: "Apply", variant: "accent" })));
  assert(accent.includes('bg-[#296A4B]'), buttonSuite, 'Accent variant mapping', 'Accent variant must map to Emerald #296A4B');

  const secondary = stripReactComments(ReactDOMServer.renderToString(React.createElement(Button, { label: "Cancel", variant: "secondary" })));
  assert(secondary.includes('border-[#001A41]'), buttonSuite, 'Secondary variant outline', 'Secondary variant must have Navy border');

  const destructive = stripReactComments(ReactDOMServer.renderToString(React.createElement(Button, { label: "Delete", variant: "destructive" })));
...
safeRender('1.5 Touch Target Sizes', () => {
  const sm = stripReactComments(ReactDOMServer.renderToString(React.createElement(Button, { label: "Small", size: "sm" })));
  assert(sm.includes('min-h-[36px]'), buttonSuite, 'Small touch target min-height', 'Small size must have min-h-[36px]');

  const md = stripReactComments(ReactDOMServer.renderToString(React.createElement(Button, { label: "Medium", size: "md" })));
  assert(md.includes('min-h-[44px]'), buttonSuite, 'Medium touch target min-height', 'Medium size must have min-h-[44px]');

  const lg = stripReactComments(ReactDOMServer.renderToString(React.createElement(Button, { label: "Large", size: "lg" })));
...
safeRender('1.6 Full Width', () => {
  const full = stripReactComments(ReactDOMServer.renderToString(React.createElement(Button, { label: "Full", fullWidth: true })));
  assert(full.includes('w-full'), buttonSuite, 'Full width class', 'Full width button must have w-full class');
});

safeRender('1.7 Button Click & Disabled Event Logic', () => {
  let clicked = false;
  let pressed = false;
  ...
  // Call handleClick directly from element props
  btnElement.props.onClick({ preventDefault: () => {} });
  assert(!clicked && !pressed, buttonSuite, 'Disabled button click suppression', 'Disabled button must not trigger onClick/onPress');

  let activeClicked = false;
  let activePressed = false;
  const activeBtn = Button({
    label: "Active Click Test",
    disabled: false,
    onPress: () => { activePressed = true; },
    onClick: () => { activeClicked = true; }
  });
  activeBtn.props.onClick({ preventDefault: () => {} });
  assert(activeClicked && activePressed, buttonSuite, 'Enabled button click execution', 'Enabled button must trigger both onClick and onPress');
});
...
safeRender('2.1 Default Card', () => {
  const html = stripReactComments(ReactDOMServer.renderToString(
    React.createElement(Card, { title: "Task Details", subtitle: "Generator repair service" }, React.createElement('p', null, 'Body content'))
  ));
...
safeRender('2.2 Interactive Card', () => {
  const html = stripReactComments(ReactDOMServer.renderToString(
    React.createElement(Card, { title: "Clickable Card", interactive: true, onClick: () => {} }, React.createElement('p', null, 'Card content'))
  ));
...
safeRender('2.3 Card Keyboard Event Navigation', () => {
  ...
  const cardElement = Card({
    title: "Keyboard Card",
    interactive: true,
    onClick: () => { cardClickedCount++; }
  });

  // Test Enter key
  cardElement.props.onKeyDown({
    key: 'Enter',
    preventDefault: () => { preventDefaultCalled = true; }
  });
  assert(cardClickedCount === 1 && preventDefaultCalled, cardSuite, 'Enter key activation', 'Pressing Enter on interactive card must trigger onClick and preventDefault');
  ...
  assert(cardClickedCount === 1 && preventDefaultCalled, cardSuite, 'Space key activation', 'Pressing Space on interactive card must trigger onClick and preventDefault');

  // Test non-trigger key (Tab)
  cardClickedCount = 0;
  cardElement.props.onKeyDown({ key: 'Tab', preventDefault: () => {} });
  assert(cardClickedCount === 0, cardSuite, 'Ignored key suppression', 'Pressing Tab should not trigger card onClick');
});

safeRender('2.4 Card Padding', () => {
  const padNone = stripReactComments(ReactDOMServer.renderToString(React.createElement(Card, { padding: "none" }, "None")));
  assert(padNone.includes('p-0'), cardSuite, 'Padding none (p-0)', 'Padding none must set p-0');

  const padLg = stripReactComments(ReactDOMServer.renderToString(React.createElement(Card, { padding: "lg" }, "Large")));
  assert(padLg.includes('p-8'), cardSuite, 'Padding lg (p-8)', 'Padding lg must set p-8');
});

safeRender('2.5 Card Header Slots', () => {
  const headerSlot = stripReactComments(ReactDOMServer.renderToString(
    React.createElement(Card, { header: React.createElement('div', { id: "custom-header" }, "Custom Header") }, React.createElement('p', null, 'Content'))
  ));
  ...
  const headerAction = stripReactComments(ReactDOMServer.renderToString(
    React.createElement(Card, { title: "Task Card", headerAction: React.createElement('button', null, 'Action') }, React.createElement('p', null, 'Content'))
  ));
...
// -------------------------------------------------------------
// 3. INPUTFIELD COMPONENT TESTS
// -------------------------------------------------------------
const inputSuite = 'InputField Component';

safeRender('3.1 Input Label & ID', () => {
  const html = stripReactComments(ReactDOMServer.renderToString(
    React.createElement(InputField, { label: "Full Name", placeholder: "Enter name" })
  ));
  ...
  assert(html.includes('Full Name'), inputSuite, 'Label text rendering', 'Label should render text node');
  assert(html.includes('uppercase'), inputSuite, 'Label uppercase class styling', 'Label should apply uppercase styling');
});

safeRender('3.2 Input Error State', () => {
  const html = stripReactComments(ReactDOMServer.renderToString(
    React.createElement(InputField, { label: "Email", error: "Invalid email address" })
  ));
...
safeRender('3.3 Input Helper Text', () => {
  const html = stripReactComments(ReactDOMServer.renderToString(
    React.createElement(InputField, { label: "Phone", helperText: "Enter 11-digit phone number" })
  ));
...
safeRender('3.4 Input Counter & MaxLength', () => {
  const rawHtml = ReactDOMServer.renderToString(
    React.createElement(InputField, { label: "Bio", value: "Hello", maxLength: 100, showCounter: true })
  );
  const html = stripReactComments(rawHtml);
...
safeRender('3.5 Input Icons', () => {
  const html = stripReactComments(ReactDOMServer.renderToString(
    React.createElement(InputField, { label: "Search", leftIcon: React.createElement('span', null, 'IconL'), rightIcon: React.createElement('span', null, 'IconR') })
  ));
...
// -------------------------------------------------------------
// 4. BUKIEPASSPORTBADGE COMPONENT TESTS
// -------------------------------------------------------------
const badgeSuite = 'BukiePassportBadge Component';

safeRender('4.1 Compact Badges', () => {
  const pro = stripReactComments(ReactDOMServer.renderToString(React.createElement(BukiePassportBadge, { tier: "Pro", compact: true })));
  ...
  const lite = stripReactComments(ReactDOMServer.renderToString(React.createElement(BukiePassportBadge, { tier: "Lite", compact: true })));
  assert(lite.includes('BUKIEPASSPORT LITE'), badgeSuite, 'Compact Lite label', 'Compact Lite badge text must match');

  const unverified = stripReactComments(ReactDOMServer.renderToString(React.createElement(BukiePassportBadge, { tier: "Unverified", compact: true })));
...
safeRender('4.2 Full Card Badge', () => {
  const html = stripReactComments(ReactDOMServer.renderToString(
    React.createElement(BukiePassportBadge, { tier: "Pro", ninVerified: true, bvnVerified: true, smartSelfieVerified: true, guarantorVerified: true, showDetails: true })
  ));
...
// -------------------------------------------------------------
// 5. ESCROWSHIELD COMPONENT TESTS
// -------------------------------------------------------------
const escrowSuite = 'EscrowShield Component';

safeRender('5.1 Escrow PENDING_AUTHORIZATION', () => {
  const html = stripReactComments(ReactDOMServer.renderToString(React.createElement(EscrowShield, { amount: 25000, status: "PENDING_AUTHORIZATION" })));
...
safeRender('5.2 Escrow HELD_IN_ESCROW', () => {
  const html = stripReactComments(ReactDOMServer.renderToString(React.createElement(EscrowShield, { amount: 50000, status: "HELD_IN_ESCROW" })));
...
safeRender('5.3 Escrow RELEASED_TO_ARTISAN', () => {
  const html = stripReactComments(ReactDOMServer.renderToString(React.createElement(EscrowShield, { amount: 75000, status: "RELEASED_TO_ARTISAN" })));
...
safeRender('5.4 Escrow REFUNDED', () => {
  const html = stripReactComments(ReactDOMServer.renderToString(React.createElement(EscrowShield, { amount: 15000, status: "REFUNDED" })));
...
safeRender('5.5 Escrow Legacy Aliases', () => {
  const preAuth = stripReactComments(ReactDOMServer.renderToString(React.createElement(EscrowShield, { amount: 10000, status: "Pre-Authorized" })));
  ...
  const captured = stripReactComments(ReactDOMServer.renderToString(React.createElement(EscrowShield, { amount: 10000, status: "Captured" })));
...
safeRender('5.6 Escrow Compact', () => {
  const compact = stripReactComments(ReactDOMServer.renderToString(React.createElement(EscrowShield, { amount: 30000, status: "HELD_IN_ESCROW", compact: true })));
