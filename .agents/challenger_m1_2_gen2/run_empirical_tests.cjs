...
const { Button } = require('../../packages/ui/src/components/Button');
const { Card } = require('../../packages/ui/src/components/Card');
const { InputField } = require('../../packages/ui/src/components/InputField');
const { BukiePassportBadge } = require('../../packages/ui/src/components/BukiePassportBadge');
const { EscrowShield } = require('../../packages/ui/src/components/EscrowShield');
...
safeRender('1.1 Default Button', () => {
  const html = ReactDOMServer.renderToString(React.createElement(Button, { label: "Submit Order" }));
...
safeRender('1.2 Loading Button', () => {
  const html = ReactDOMServer.renderToString(React.createElement(Button, { label: "Processing", isLoading: true }));
...
safeRender('1.3 Disabled Button', () => {
  const html = ReactDOMServer.renderToString(React.createElement(Button, { label: "Disabled Action", disabled: true }));
...
safeRender('1.4 Button Variants', () => {
  const emerald = ReactDOMServer.renderToString(React.createElement(Button, { label: "Pay Now", variant: "emerald" }));
  assert(emerald.includes('bg-[#296A4B]'), buttonSuite, 'Emerald variant color token', 'Emerald variant must use #296A4B');

  const accent = ReactDOMServer.renderToString(React.createElement(Button, { label: "Apply", variant: "accent" }));
  assert(accent.includes('bg-[#296A4B]'), buttonSuite, 'Accent variant mapping', 'Accent variant must map to Emerald #296A4B');

  const secondary = ReactDOMServer.renderToString(React.createElement(Button, { label: "Cancel", variant: "secondary" }));
  assert(secondary.includes('border-[#001A41]'), buttonSuite, 'Secondary variant outline', 'Secondary variant must have Navy border');

  const destructive = ReactDOMServer.renderToString(React.createElement(Button, { label: "Delete", variant: "destructive" }));
...
safeRender('1.5 Touch Target Sizes', () => {
  const sm = ReactDOMServer.renderToString(React.createElement(Button, { label: "Small", size: "sm" }));
  assert(sm.includes('min-h-[36px]'), buttonSuite, 'Small touch target min-height', 'Small size must have min-h-[36px]');

  const md = ReactDOMServer.renderToString(React.createElement(Button, { label: "Medium", size: "md" }));
  assert(md.includes('min-h-[44px]'), buttonSuite, 'Medium touch target min-height', 'Medium size must have min-h-[44px]');

  const lg = ReactDOMServer.renderToString(React.createElement(Button, { label: "Large", size: "lg" }));
...
safeRender('1.6 Full Width', () => {
  const full = ReactDOMServer.renderToString(React.createElement(Button, { label: "Full", fullWidth: true }));
...
safeRender('2.3 Card Padding', () => {
  const padNone = ReactDOMServer.renderToString(React.createElement(Card, { padding: "none" }, "None"));
  assert(padNone.includes('p-0'), cardSuite, 'Padding none (p-0)', 'Padding none must set p-0');

  const padLg = ReactDOMServer.renderToString(React.createElement(Card, { padding: "lg" }, "Large"));
...
safeRender('4.1 Compact Badges', () => {
  const pro = ReactDOMServer.renderToString(React.createElement(BukiePassportBadge, { tier: "Pro", compact: true }));
  ...
  const lite = ReactDOMServer.renderToString(React.createElement(BukiePassportBadge, { tier: "Lite", compact: true }));
  assert(lite.includes('BUKIEPASSPORT LITE'), badgeSuite, 'Compact Lite label', 'Compact Lite badge text must match');

  const unverified = ReactDOMServer.renderToString(React.createElement(BukiePassportBadge, { tier: "Unverified", compact: true }));
...
safeRender('5.1 Escrow PENDING_AUTHORIZATION', () => {
  const html = ReactDOMServer.renderToString(React.createElement(EscrowShield, { amount: 25000, status: "PENDING_AUTHORIZATION" }));
...
safeRender('5.2 Escrow HELD_IN_ESCROW', () => {
  const html = ReactDOMServer.renderToString(React.createElement(EscrowShield, { amount: 50000, status: "HELD_IN_ESCROW" }));
...
safeRender('5.3 Escrow RELEASED_TO_ARTISAN', () => {
  const html = ReactDOMServer.renderToString(React.createElement(EscrowShield, { amount: 75000, status: "RELEASED_TO_ARTISAN" }));
...
safeRender('5.4 Escrow REFUNDED', () => {
  const html = ReactDOMServer.renderToString(React.createElement(EscrowShield, { amount: 15000, status: "REFUNDED" }));
...
safeRender('5.5 Escrow Legacy Aliases', () => {
  const preAuth = ReactDOMServer.renderToString(React.createElement(EscrowShield, { amount: 10000, status: "Pre-Authorized" }));
  ...
  const captured = ReactDOMServer.renderToString(React.createElement(EscrowShield, { amount: 10000, status: "Captured" }));
...
safeRender('5.6 Escrow Compact', () => {
  const compact = ReactDOMServer.renderToString(React.createElement(EscrowShield, { amount: 30000, status: "HELD_IN_ESCROW", compact: true }));
