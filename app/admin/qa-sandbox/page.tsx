...
export default function QASandboxPage() {
    ...
    return (
      <main className="min-h-screen bg-brand-bg text-brand-navy py-12 px-4 flex flex-col items-center justify-center">
        ...
        >
          ...
          <h1 className="text-xl font-bold text-brand-navy tracking-tight font-display">Access Gate Restrict</h1>
  ...
  return (
    <main className="min-h-screen bg-brand-bg text-brand-navy py-8 px-4 flex flex-col font-sans">
      <div className="max-w-4xl w-full mx-auto space-y-6" id="qa-sandbox-panel-container">
        ...
        <div className="flex justify-between items-center bg-brand-surface p-4 rounded-2xl border border-brand-border/60 shadow-xs">
          ...
          <span className="text-[10px] uppercase font-mono tracking-widest text-brand-green font-semibold flex items-center gap-1.5 bg-brand-green/10 px-3 py-1.5 rounded-xl border border-brand-green/20">
            <ShieldCheck className="w-4 h-4 fill-brand-green text-brand-bg" />
            <span>CEO Mode Authorized</span>
          </span>
        </div>

        {/* Header Hero */}
        <div className="bg-brand-surface p-8 rounded-2xl border border-brand-border/60 shadow-xs relative overflow-hidden" id="sandbox-header-card">
          <div className="absolute right-0 top-0 bottom-0 w-1/3 bg-gradient-to-l from-brand-green/5 to-transparent pointer-events-none" />
          <h1 className="text-3xl font-bold text-brand-navy tracking-tight leading-none mb-2 font-display">
            Isomorphic QA Sandbox Terminal
          </h1>
          <p className="text-xs text-brand-navy/60 max-w-xl leading-relaxed">
          ...
          <AnimatePresence>
            {statusMessage && (
              <motion.div 
                ...
                className={`mt-4 p-3.5 rounded-xl text-xs font-semibold border flex items-center gap-2 font-display ${
                  statusType === 'success' 
                ...
                id="sandbox-toast"
              >
                {statusType === 'success' ? <CheckCircle className="w-4 h-4 text-brand-green" /> : <XCircle className="w-4 h-4 text-red-600" />}
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6" id="sandbox-actions-grid">
          ...
          <div className="bg-brand-surface p-6 rounded-2xl border border-brand-border/60 shadow-xs flex flex-col justify-between" id="sandbox-db-probe-card">
            <div>
              <div className="flex items-center gap-2 mb-3">
                <div className="p-2.5 bg-brand-navy/5 text-brand-navy border border-brand-border/60 rounded-xl">
                  <Database className="w-5 h-5" />
                </div>
                <div>
                  <h2 className="text-sm font-bold text-brand-navy uppercase tracking-tight font-display">Database Connectivity Test</h2>
                  <p className="text-[10px] text-brand-navy/40 font-medium">Auto-write, verify-read, and secure-erase cycle</p>
                </div>
              </div>
              <p className="text-xs text-brand-navy/60 leading-relaxed mb-4">
            ...
            <div className="space-y-4">
              {dbTestLogs.length > 0 && (
                <div className="bg-brand-navy p-3 rounded-xl border border-brand-navy/80 text-[10px] text-white/85 font-mono space-y-1.5 h-36 overflow-y-auto block leading-normal">
                  <div className="text-brand-green uppercase font-bold border-b border-white/10 pb-1 flex justify-between items-center">
              ...
              <button
                ...
                disabled={dbTesting}
                className="w-full bg-brand-green hover:bg-brand-green/90 disabled:bg-brand-navy/20 text-white text-xs font-bold uppercase tracking-wider py-3.5 px-4 rounded-xl transition-all cursor-pointer flex items-center justify-center gap-2 shadow-xs active:scale-[0.98] font-display"
          ...
          <div className="bg-brand-surface p-6 rounded-2xl border border-brand-border/60 shadow-xs flex flex-col justify-between" id="sandbox-env-probe-card">
            <div>
              <div className="flex items-center gap-2 mb-3">
                <div className="p-2.5 bg-brand-navy/5 text-brand-navy border border-brand-border/60 rounded-xl">
                ...
                <div>
                  <h2 className="text-sm font-bold text-brand-navy uppercase tracking-tight font-display">Active Environment Audit</h2>
                  <p className="text-[10px] text-brand-navy/40 font-medium font-mono">Verify server credentials state cleanly</p>
                </div>
              </div>
              <p className="text-xs text-brand-navy/60 leading-relaxed mb-4">
            ...
            <div className="space-y-4">
              {envStates && (
                <div className="border border-brand-border/60 p-3.5 rounded-xl space-y-2 bg-brand-bg font-mono text-[10px]" id="sandbox-env-states">
                  <div className="flex justify-between items-center">
                    <span className="text-brand-navy/60 font-semibold">PAYSTACK_SECRET_KEY</span>
                    {envStates.PAYSTACK_SECRET_KEY ? (
                      <span className="bg-brand-green/10 text-brand-green text-[9px] uppercase px-1.5 py-0.5 rounded font-black">LOADED</span>
                    ) : (
                      <span className="bg-red-100 text-red-800 text-[9px] uppercase px-1.5 py-0.5 rounded font-black">MISSING</span>
                    )}
                  </div>
                  <div className="flex justify-between items-center">
                    <span className="text-brand-navy/60 font-semibold">TELEGRAM_BOT_TOKEN</span>
                  ...
                  <div className="flex justify-between items-center">
                    <span className="text-brand-navy/60 font-semibold">TELEGRAM_CHANNEL_ID</span>
                  ...
                  <div className="flex justify-between items-center">
                    <span className="text-brand-navy/60 font-semibold">PAYSTACK_PUBLIC_KEY</span>
              ...
              <button
                ...
                disabled={envLoading}
                className="w-full bg-brand-navy hover:bg-brand-navy/90 disabled:bg-brand-navy/20 text-white text-xs font-bold uppercase tracking-wider py-3.5 px-4 rounded-xl transition-all cursor-pointer flex items-center justify-center gap-2 shadow-xs active:scale-[0.98] font-display"
        ...
        <div className="bg-brand-surface p-6 rounded-2xl border-2 border-dashed border-red-200/60 shadow-xs flex flex-col md:flex-row justify-between items-start md:items-center gap-4" id="sandbox-purge-card">
          <div className="space-y-1 max-w-xl">
            <h2 className="text-sm font-bold text-red-700 uppercase tracking-tight flex items-center gap-1.5 font-display">
              <Trash2 className="w-4 h-4 text-red-500" />
              <span>Full Safe Purge of Sandbox Test Records</span>
            </h2>
            <p className="text-xs text-brand-navy/60 leading-relaxed">
              Purges all jobs and linked messaging logs that contain the string <strong className="text-brand-navy font-bold">[TEST]</strong> in their title or content message fields. This allows risk-free end-to-end sandbox evaluations during environment staging.
            </p>
            {cleanupResult && (
              <p className="text-[10px] font-mono text-brand-green font-bold pt-1">
          ...
          <button
            ...
            className="shrink-0 w-full md:w-auto bg-red-600 hover:bg-red-700 disabled:bg-brand-navy/20 text-white text-xs font-extrabold uppercase tracking-wider py-3.5 px-6 rounded-xl transition-all cursor-pointer flex items-center justify-center gap-2 font-display"
